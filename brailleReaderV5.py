...
def findPoint(pointGroup, points, roiTLC):
    ...
    for point in points:
        if not point.haveAGroup:
            ...
            if areCloseEnough(parentPoint, point):
                ...
                if len(pointGroup) < 5 :
                    findPoint(pointGroup, points, roiTLC)
...
def create_braille_chars(point_groups):
    ...
    for point_group in point_groups:
        x, y, width, height = findPointBox(point_group)
...
def find_point_groups(points, roiTLC):
    pointGroups = []
...
def translate(image, roi, roiTLC, points, mythreshold):
    ...
    pointGroups = find_point_groups(points, roiTLC)
    
    braille_chars = create_braille_chars(pointGroups)
