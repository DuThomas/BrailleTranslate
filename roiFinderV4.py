...
from math import *
import translator
import brailleReaderV4debug as brailleReaderV4
...
while True:
    ...
    points = list(brailleReaderV4.Point(contour) for contour in contours)
    bestPoints = []
    ...
    if len(bestPoints) > 1:
        ...
        if (not pointsBox[1] == pointsBox[3]) and (not pointsBox[0] == pointsBox[2] or True):
            ...
            # print(pointsBox, image.shape, (pointsBox[2] - pointsBox[0], pointsBox[3] - pointsBox[1]))
            cv2.imshow("ROI", roi) 
            cv2.imshow("Result", brailleReaderV4.translate(image.copy(), roi, (pointsBox[0], pointsBox[1]), bestPoints, mythreshold))
