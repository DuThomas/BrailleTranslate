...
def translate(image, roi, roiTLC, contours, mythreshold):
    ...
    for i in range(len(newGroupeBoxes)):
        ...
        margin = int(width / 4)
        tlCorner2 = (tlCorner[0] - margin + roiTLC[0], tlCorner[1] - margin + roiTLC[1])
        brCorner2 = (tlCorner[0] + width + margin + roiTLC[0], tlCorner[1] + height + margin + roiTLC[1])

        tlCorner = (tlCorner[0] - margin, tlCorner[1] - margin)
        brCorner = (tlCorner[0] + width + margin, tlCorner[1] + height + margin)
