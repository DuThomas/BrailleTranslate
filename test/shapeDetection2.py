...
while True:
    ...
    while row < ymax:
        col = xmin
        while col < xmax:
            oneBrailleChar = threshold[row:row + int(2.5 * yDistances[0]), col:col + int(1.5 * xDistances[0])]
            # cv2.imshow("BrailleChar", oneBrailleChar)
            # cv2.waitKey(0)
            cv2.putText(threshold, main.translate(oneBrailleChar), (col + int(1.5 * xDistances[0] / 2), row + int(2.5 * yDistances[0])), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (100, 100, 100), 2)
